...
class BatchTransaction {
  ...
  factory BatchTransaction.fromJson(Map<String, dynamic> json) =>
      BatchTransaction(
        ...
        mortalityReasons:
            Map<String, int>.from(json['mortalityReasons'] as Map),
...
class _DashboardScreenState extends State<DashboardScreen>
  ...
  void _handleTabSelection() {
    ...
    if (!_recharged && _tabController.index != 0) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text(
              'Please complete a credit recharge to unlock User and Batch tabs!'),
  ...
  void _loadUserRoleAndName() {
    ...
    final displayName =
        LocalStorageService.getString('display_name') ?? 'Sagar Godbole';
    final farmerId = LocalStorageService.getString('farmer_id') ?? '1';
    final country = LocalStorageService.getString('country') ?? 'India';
    final config =
        _countryPurchaseConfigs[country] ?? _countryPurchaseConfigs['India']!;
  ...
  void _loadDataFromStorage() {
    ...
    if (txJson != null && txJson.isNotEmpty) {
      try {
        ...
        setState(() {
          _transactions.clear();
          _transactions.addAll(decoded.map(
              (t) => BatchTransaction.fromJson(t as Map<String, dynamic>)));
        });
      } catch (e) {
  ...
  // ---- RECHARGE ----
  void _showRechargeWizard() {
    context.push('/recharge').then((_) {
      // Refresh balances from storage after returning from the recharge flow
      if (mounted) {
        setState(() {
          _broilerBalance = LocalStorageService.getInt('broiler_balance') ?? 0;
          _layerBalance = LocalStorageService.getInt('layer_balance') ?? 0;
  ...
  void _showUserForm({FarmerUser? user, int? index}) {
    ...
    final farmerIdCtrl =
        TextEditingController(text: user?.farmerId ?? loggedInFarmerId);
                  ...
                  onPressed: () {
                    if (formKey.currentState!.validate()) {
                      ...
                      ScaffoldMessenger.of(context).showSnackBar(SnackBar(
                        content:
                            Text(isEdit ? 'User updated!' : 'User created!'),
  ...
  void _adjustBalance(String module, int delta) {
    setState(() {
      if (module == 'Broiler') {
        _broilerBalance += delta;
      } else if (module == 'Layer') {
        _layerBalance += delta;
      } else if (module == 'Breeder') {
        _breederBalance += delta;
      }
    });
  ...
  void _showBatchForm({PoultryBatch? batch, int? index}) {
    ...
    final contractFarmerCtrl =
        TextEditingController(text: batch?.contractFarmerName ?? '');
    final contractShedCtrl =
        TextEditingController(text: batch?.contractShedName ?? '');
    final liftingAgeCtrl =
        TextEditingController(text: batch?.stdLiftingAge.toString() ?? '42');
    final chicksQtyCtrl =
        TextEditingController(text: batch?.chicksQuantity.toString() ?? '');
    final birdRateCtrl =
        TextEditingController(text: batch?.birdRate.toString() ?? '');
                  ...
                  children: [
                    ...
                    DropdownButtonFormField<String>(
                      ...
                      items: ['Broiler', 'Layer', 'Breeder'].map((m) {
                        ...
                        final available =
                            isEdit && batch.module == m ? bal + 1 : bal;
                      ...
                      validator: (v) {
                        ...
                        final available =
                            isEdit && batch.module == v ? bal + 1 : bal;
                        if (!isEdit && available <= 0) {
                          return 'No credits left for $v. Please recharge.';
                        }
                        return null;
                      },
                    ),
                    ...
                    if (batchType == 'Own') ...[
                      ...
                      farmers.isEmpty
                          ? Container(
                              ...
                              child: const Text(
                                  'No farmers found. Please create a user first.',
                                  style: TextStyle(
                          ...
                          : DropdownButtonFormField<String>(
                              ...
                              items: farmers
                                  .map((f) => DropdownMenuItem(
                                        value: f.userCode,
                                        child:
                                            Text('${f.name} (${f.userCode})'),
                                      ))
                                  .toList(),
                              onChanged: (v) => setBS(() {
                                linkedFarmerCode = v;
                                linkedFarmerName = farmers
                                    .firstWhere((f) => f.userCode == v)
                                    .name;
                              }),
                              validator: (v) =>
                    ...
                    DropdownButtonFormField<String>(
                      ...
                      items: const [
                        DropdownMenuItem(
                            value: 'COBB 430', child: Text('COBB 430')),
                        DropdownMenuItem(
                            value: 'COBB 550', child: Text('COBB 550')),
                      ],
                      onChanged: (v) {
                        if (v != null) setBS(() => breedName = v);
                      },
                    ),
                    const SizedBox(height: 16),
                      ...
                      child: AbsorbPointer(
                        child: TextFormField(
                          decoration: InputDecoration(
                            ...
                            hintText:
                                '${firstDayDate.day}/${firstDayDate.month}/${firstDayDate.year}',
                          ),
                          controller: TextEditingController(
                            text:
                                '${firstDayDate.day.toString().padLeft(2, '0')}/${firstDayDate.month.toString().padLeft(2, '0')}/${firstDayDate.year}',
                          ),
                        ),
                      ),
                    ),
                ...
                onPressed: () {
                  if (formKey.currentState!.validate()) {
                    final newBatch = PoultryBatch(
                      id: isEdit
                          ? batch.id
                          : 'BCH-${Random().nextInt(90000) + 10000}',
                      batchName: batchNameCtrl.text.trim(),
                      module: selectedModule,
                      type: batchType,
                      linkedFarmerCode:
                          batchType == 'Own' ? linkedFarmerCode : null,
                      linkedFarmerName:
                          batchType == 'Own' ? linkedFarmerName : null,
                      contractFarmerName: batchType == 'Contract'
                          ? contractFarmerCtrl.text.trim()
                          : null,
                      contractShedName: batchType == 'Contract'
                          ? contractShedCtrl.text.trim()
                          : null,
                      breedName: breedName,
                      date: selectedDate,
                      firstDayDate: firstDayDate,
                      stdLiftingAge:
                          int.tryParse(liftingAgeCtrl.text.trim()) ?? 42,
                      chicksQuantity:
                          int.tryParse(chicksQtyCtrl.text.trim()) ?? 0,
                      birdRate:
                          double.tryParse(birdRateCtrl.text.trim()) ?? 0.0,
                    );
                    setState(() {
                      if (isEdit && index != null) {
                    ...
                    ScaffoldMessenger.of(context).showSnackBar(SnackBar(
                      content: Text(isEdit
                          ? 'Batch updated!'
                          : 'Batch "${newBatch.batchName}" created!'),
                      backgroundColor: AppTheme.success,
                    ));
                  }
                },
  ...
  void _confirmDeleteBatch(int index) {
    showDialog(
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: Text(
            'Delete batch "${_batches[index].batchName}"? This cannot be undone.'),
        actions: [
          TextButton(
              onPressed: () => Navigator.pop(ctx),
              child:
          ...
          TextButton(
            onPressed: () {
              ...
              Navigator.pop(ctx);
              ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
                  content: Text('Batch deleted. Credit restored.'),
                  backgroundColor: AppTheme.success));
            },
            child:
                const Text('Delete', style: TextStyle(color: AppTheme.error)),
    ...
    return SingleChildScrollView(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.stretch, children: [
          ...
          child: Container(
            decoration: BoxDecoration(
              ...
              gradient: const LinearGradient(
                  colors: [Colors.white, AppTheme.grey100],
        ...
        ElevatedButton.icon(
          ...
          label: const Text('Recharge Accounts / Buy Batches',
                ...
                child: Row(children: [
                  ...
                  Expanded(
                      child: Column(
                        ...
                        const SizedBox(height: 4),
                        Text('ID: ${user.farmerId}  •  Code: ${user.userCode}',
                            style: const TextStyle(
        ...
        child:
          ...
          Container(
            ...
            decoration: BoxDecoration(
                color: AppTheme.successLight,
                borderRadius: BorderRadius.circular(8)),
            child: Text('${_batches.length} Batches',
                style: const TextStyle(
                    color: AppTheme.successDark,
                    fontSize: 11,
                    fontWeight: FontWeight.bold)),
                ...
                child: Column(
                    ...
                    children: [
                      ...
                      const Text('No batches created yet',
                          style:
                              TextStyle(color: AppTheme.grey500, fontSize: 15)),
                      const SizedBox(height: 6),
                      const Text('Tap + to create your first batch',
                          style:
                              TextStyle(color: AppTheme.grey400, fontSize: 12)),
                      ...
                      child: Column(
                          ...
                          children: [
                            ...
                            Row(children: [
                              ...
                              IconButton(
                                  icon: const Icon(Icons.edit_outlined,
                                      color: AppTheme.primaryRed, size: 20),
                                  onPressed: () =>
                                      _showBatchForm(batch: b, index: index),
                                  tooltip: 'Edit'),
                              IconButton(
                                  icon: const Icon(Icons.delete_outline,
                                      color: AppTheme.error, size: 20),
                                  onPressed: () => _confirmDeleteBatch(index),
                                  tooltip: 'Delete'),
                            ]),
                            ...
                            if (b.type == 'Own' && b.linkedFarmerName != null)
                              _batchInfoRow(Icons.person_outline, 'Farmer',
                                  '${b.linkedFarmerName} (${b.linkedFarmerCode})'),
                            if (b.type == 'Contract') ...[
                              _batchInfoRow(
                                  Icons.person_add_outlined,
                                  'Contract Farmer',
                                  b.contractFarmerName ?? '-'),
                              _batchInfoRow(Icons.home_work_outlined, 'Shed',
                                  b.contractShedName ?? '-'),
                            ],
                            ...
                            Wrap(spacing: 8, runSpacing: 8, children: [
                              _batchChip(
                                  ...
                                  b.module == 'Broiler'
                                      ...
                                      : b.module == 'Layer'
                                          ...
                                          : Colors.redAccent),
                              _batchChip(Icons.cruelty_free_outlined,
                                  b.breedName, Colors.brown),
                              _batchChip(Icons.egg_alt_outlined,
                                  '${b.chicksQuantity} Chicks', Colors.teal),
                              _batchChip(
                                  Icons.payments_outlined,
                                  '$_currencySymbol${b.birdRate.toStringAsFixed(2)}/bird',
                                  Colors.green),
                              _batchChip(Icons.timer_outlined,
                                  'Lift: ${b.stdLiftingAge}d', Colors.purple),
                              _batchChip(
                              ...
                              _batchChip(
                                  ...
                                  'Day 1: ${b.firstDayDate.day}/${b.firstDayDate.month}/${b.firstDayDate.year}',
                                  Colors.indigo),
                            ]),
                          ]),
                    ),
  ...
  Widget _buildFarmerDashboard() {
    final assignedBatches =
        _batches.where((b) => b.linkedFarmerCode == _userCode).toList();

    // Sum up chicks and transaction metrics
    int totalInitialChicks =
        assignedBatches.fold(0, (sum, b) => sum + b.chicksQuantity);
    ...
    for (var b in assignedBatches) {
      ...
      for (var t in txs) {
        totalMortalityCount +=
            t.mortalityReasons.values.fold(0, (sum, val) => sum + val);
        totalFeedBags += t.feedItems.values.fold(0.0, (sum, val) => sum + val);
      }
    }

    int totalLiveQuantity = totalInitialChicks - totalMortalityCount;
    double mortalityRate = totalInitialChicks > 0
        ? (totalMortalityCount / totalInitialChicks) * 100
        : 0.0;
        ...
        child: Column(
          ...
          children: [
              ...
              children: [
                ...
                Expanded(
                  child: Card(
                    ...
                    child: InkWell(
                      onTap: () =>
                          _showAnalyticsScreen(context, assignedBatches),
                      borderRadius: BorderRadius.circular(12),
                      child: Padding(
                        padding: const EdgeInsets.all(16),
                        child: Row(
            ...
            Row(
              mainAxisAlignment: MainAxisAlignment.spaceBetween,
              children: [
                const Text('My Assigned Batches',
                    style: TextStyle(
                        fontSize: 16,
                        fontWeight: FontWeight.bold,
                        color: AppTheme.grey800)),
                Container(
                  ...
                  decoration: BoxDecoration(
                      color: const Color(0xFF0D8B60).withValues(alpha: 0.1),
                      borderRadius: BorderRadius.circular(8)),
                  child: Text(
            ...
            if (assignedBatches.isEmpty)
                ...
                child: Column(
                  children: [
                    const Icon(Icons.layers_clear_outlined,
                        size: 56, color: AppTheme.grey300),
                    const SizedBox(height: 14),
                    const Text('No batches assigned to you yet',
                        style:
                            TextStyle(color: AppTheme.grey500, fontSize: 15)),
                    const SizedBox(height: 4),
                    const Text('Contact Admin to link you to a batch.',
                        style:
                            TextStyle(color: AppTheme.grey400, fontSize: 12)),
                  ],
                ),
              ...
              ListView.builder(
                ...
                itemBuilder: (context, index) {
                  ...
                  final bTxs =
                      _transactions.where((t) => t.batchId == b.id).toList();
                  int bMortality = bTxs.fold(
                      0,
                      (sum, t) =>
                          sum +
                          t.mortalityReasons.values
                              .fold(0, (s, val) => s + val));
                  double bFeed = bTxs.fold(
                      0.0,
                      (sum, t) =>
                          sum +
                          t.feedItems.values.fold(0.0, (s, val) => s + val));
                            ...
                            Wrap(
                              ...
                              children: [
                                ...
                                _batchChip(Icons.cruelty_free_outlined,
                                    b.breedName, Colors.brown),
                                _batchChip(Icons.egg_alt_outlined,
                                    '$bLiveQty Live Birds', Colors.teal),
                                _batchChip(Icons.heart_broken_outlined,
                                    '$bMortality Dead', AppTheme.primaryRed),
                                _batchChip(
                                    Icons.grass_outlined,
                                    '${bFeed.toStringAsFixed(1)} KG Feed',
                                    Colors.orange),
                                _batchChip(Icons.timer_outlined,
                                    'Lift: ${b.stdLiftingAge}d', Colors.purple),
                                _batchChip(
      ...
      child: Padding(
        ...
        child: Column(
          children: [
            ...
            const SizedBox(height: 2),
            Text(subtitle,
                style: const TextStyle(fontSize: 9, color: AppTheme.grey400)),
          ],
        ),
      ),
    );
  ...
  void _showReportsScreen(
      BuildContext context, List<PoultryBatch> assignedBatches) {
    Navigator.push(
      context,
      MaterialPageRoute(
...
class _RechargeWizardDialogState extends State<RechargeWizardDialog> {
  ...
  Widget _buildSelect() {
    return Form(
      ...
      child: Column(
          ...
          children: [
            Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
              const Text('Select Module & Batches',
                  style: TextStyle(
                      fontSize: 17,
                      fontWeight: FontWeight.bold,
                      color: AppTheme.grey800)),
            ...
            const Text('Select Module',
                style: TextStyle(
                    fontWeight: FontWeight.bold, color: AppTheme.grey700)),
            const SizedBox(height: 6),
            DropdownButtonFormField<String>(
              value: _selectedModule,
              decoration: const InputDecoration(
                  prefixIcon:
                      Icon(Icons.category_outlined, color: AppTheme.primaryRed),
                  contentPadding:
                      EdgeInsets.symmetric(horizontal: 16, vertical: 12)),
              items: const [
            ...
            TextFormField(
              ...
              decoration: const InputDecoration(
                  prefixIcon:
                      Icon(Icons.numbers_outlined, color: AppTheme.primaryRed),
                  hintText: 'e.g. 5',
                  contentPadding:
                      EdgeInsets.symmetric(horizontal: 16, vertical: 12)),
              validator: (v) {
            ...
            if (_cart.isNotEmpty) ...[
              ...
              OutlinedButton(
                  ...
                  child: Text('View Cart (${_cart.length})')),
            ],
          ]),
    );
  }
  ...
  Widget _buildCart() {
    return Column(
        ...
        children: [
          Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
            Row(children: [
              const Icon(Icons.shopping_cart, color: AppTheme.primaryRed),
              const SizedBox(width: 8),
                ...
                itemBuilder: (_, i) {
                  ...
                  return Card(
                    ...
                    child: ListTile(
                      ...
                      title: Text('${item.module} Module',
                          style: const TextStyle(
                              fontWeight: FontWeight.bold, fontSize: 13)),
                      subtitle: Text(
                          '${item.count} Batches @ $_currencySymbol${_pricePerBatch.toStringAsFixed(0)}'),
  ...
  Widget _buildInvoice() {
    ...
    return Column(
        ...
        children: [
          Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
            const Row(children: [
              Icon(Icons.receipt_long, color: AppTheme.primaryRed),
              SizedBox(width: 8),
              Text('Invoice',
                  style: TextStyle(
                      fontSize: 17,
              ...
              child: Column(
                ...
                children: [
                  SwitchListTile(
                    ...
                    subtitle: const Text('IGST 18% instead of CGST+SGST 9%+9%',
                  ...
                  Column(
                    ...
                    children: [
                      ...
                      SingleChildScrollView(
                        ...
                        child: Row(
                          children: [
                            _buildCouponCard('FARMER50', '50% OFF',
                                'Save 50% on all licenses'),
                            _buildCouponCard(
                                'FREESHED',
                                '$_currencySymbol${_flatDiscountAmount.toStringAsFixed(0)} OFF',
                                _flatDiscountText),
                            _buildCouponCard('POULTRYOS', '20% OFF',
                                'Flat 20% discount on total cost'),
                            _buildCouponCard('WELCOME10', '10% OFF',
                                'Flat 10% discount for first recharge'),
                          ],
                        ),
                      ),
                    ],
                  ...
                  Container(
                    ...
                    child: Column(
                        ...
                        children: [
                          ...
                          if (_appliedCoupon != null)
                            _invoiceRow('Coupon Discount ($_appliedCoupon)',
                                '-$_currencySymbol${_discountAmount.toStringAsFixed(2)}',
                                isDiscount: true),
                          _invoiceRow('CGST (9%)',
                              '$_currencySymbol${cgst.toStringAsFixed(2)}',
                              dim: _isInterstate),
                          _invoiceRow('SGST (9%)',
                              '$_currencySymbol${sgst.toStringAsFixed(2)}',
                              dim: _isInterstate),
                          _invoiceRow('IGST (18%)',
                              '$_currencySymbol${igst.toStringAsFixed(2)}',
                              dim: !_isInterstate),
                          const Divider(height: 14),
                          Row(
                              mainAxisAlignment: MainAxisAlignment.spaceBetween,
  ...
  Widget _invoiceRow(String label, String value,
      {bool dim = false, bool isDiscount = false}) {
    ...
    return Column(
        ...
        children: [
          const Icon(Icons.check_circle_outline_rounded,
              size: 70, color: AppTheme.primaryRed),
          const SizedBox(height: 14),
          const Center(
          ...
          Container(
            ...
            child: Column(children: [
              ...
              const Divider(height: 14),
              ..._cart.map((item) => _receiptRow(
                  '${item.module} License', '${item.count} Batches')),
              const Divider(height: 10),
              _receiptRow('Subtotal',
                  '$_currencySymbol${_subtotal.toStringAsFixed(2)}'),
              if (_appliedCoupon != null)
                _receiptRow('Coupon Discount ($_appliedCoupon)',
                    '-$_currencySymbol${_discountAmount.toStringAsFixed(2)}'),
              if (!_isInterstate) ...[
                _receiptRow(
                    'CGST (9%)', '$_currencySymbol${cgst.toStringAsFixed(2)}'),
                _receiptRow(
                    'SGST (9%)', '$_currencySymbol${sgst.toStringAsFixed(2)}'),
              ] else
                _receiptRow(
                    'IGST (18%)', '$_currencySymbol${igst.toStringAsFixed(2)}'),
              const Divider(height: 10),
              _receiptRow(
                  'Total Paid', '$_currencySymbol${total.toStringAsFixed(2)}',
                  bold: true),
            ]),
          ),
          const SizedBox(height: 20),
...
double getStandardFCR(int ageDays) {
  ...
  if (ageDays <= 21)
    return 1.15 + ((ageDays - 14) / 7.0) * 0.15; // 1.15 -> 1.30
  if (ageDays <= 28)
    return 1.30 + ((ageDays - 21) / 7.0) * 0.15; // 1.30 -> 1.45
  if (ageDays <= 35)
    return 1.45 + ((ageDays - 28) / 7.0) * 0.15; // 1.45 -> 1.60
...
class BatchTransactionsScreen extends StatefulWidget {
  ...
  @override
  State<BatchTransactionsScreen> createState() =>
      _BatchTransactionsScreenState();
}
...
class _BatchTransactionsScreenState extends State<BatchTransactionsScreen> {
  ...
  void initState() {
    super.initState();
    _localTxs =
        widget.transactions.where((t) => t.batchId == widget.batch.id).toList();
  }

  void _refreshTransactions() {
    setState(() {
      _localTxs = widget.transactions
          .where((t) => t.batchId == widget.batch.id)
          .toList();
    });
  }
            ...
            onPressed: () {
              showDialog(
                ...
                builder: (ctx) => AlertDialog(
                  ...
                  content: const Text(
                      'No new alerts or notifications for this batch.'),
                  actions: [
                    TextButton(
                      onPressed: () => Navigator.pop(ctx),
                      child: const Text('OK'),
              ...
              if (_localTxs.isEmpty)
                Padding(
                  ...
                  child: Column(
                    children: [
                      ...
                      const SizedBox(height: 4),
                      const Text(
                          'Click the + button below to add today\'s update.',
                          style:
                              TextStyle(color: AppTheme.grey400, fontSize: 12)),
                    ],
                  ),
                ...
                ListView.builder(
                  ...
                  itemBuilder: (ctx, idx) {
                    ...
                    final txMortality = tx.mortalityReasons.values
                        .fold(0, (sum, val) => sum + val);
                    final txFeed =
                        tx.feedItems.values.fold(0.0, (sum, val) => sum + val);
                            ...
                            Row(
                              ...
                              children: [
                                ...
                                PopupMenuButton<String>(
                                  ...
                                  onSelected: (val) {
                                    if (val == 'view') {
                                      _showBatchLoggingDialog(widget.batch,
                                          existingTx: tx, readOnly: true);
                                    } else if (val == 'edit') {
                                      _showBatchLoggingDialog(widget.batch,
                                          existingTx: tx, readOnly: false);
                                    }
                                  },
                                  itemBuilder: (ctx) => [
                                    const PopupMenuItem(
  ...
  void _showBatchLoggingDialog(PoultryBatch batch,
      {BatchTransaction? existingTx, bool readOnly = false}) {
    DateTime txDate = existingTx != null ? existingTx.date : DateTime.now();
    final bodyWeightCtrl = TextEditingController(
        text:
            existingTx != null ? existingTx.bodyWeight.toStringAsFixed(0) : '');
    final formKey = GlobalKey<FormState>();

    // Calculate initial metrics
    final bTxs =
        widget.transactions.where((t) => t.batchId == batch.id).toList();
    int totalPrevMortality = bTxs.fold(
        0,
        (sum, t) =>
            sum + t.mortalityReasons.values.fold(0, (s, val) => s + val));

    // If editing existing, adjust prev mortality to not double count this txn
    if (existingTx != null) {
      totalPrevMortality -=
          existingTx.mortalityReasons.values.fold(0, (sum, val) => sum + val);
    }
    ...
    final Map<String, TextEditingController> mortCtrls = {
      for (var r in mortalityReasons)
        r: TextEditingController(
            text: existingTx != null
                ? (existingTx.mortalityReasons[r] ?? 0).toString()
                : '0')
    };
    final Map<String, TextEditingController> feedCtrls = {
      for (var f in feedItems)
        f: TextEditingController(text: getFeedValue(f).toStringAsFixed(1))
    };

    List<String> capturedMedia =
        existingTx != null ? List<String>.from(existingTx.mediaPaths) : [];
                      ...
                      Center(
                        child: Column(
                          ...
                          children: [
                            ...
                            Text(
                              isCapturingVideo
                                  ? 'RECORDING POULTRY VIDEO'
                                  : 'ALIGN CAMERA WITH POULTRY FLOCK',
                              style: TextStyle(
                            ...
                            GestureDetector(
                              onTap: () {
                                ...
                                ScaffoldMessenger.of(context).showSnackBar(
                                  const SnackBar(
                                    content:
                                        Text('Mock Live Capture Successful!'),
                                    backgroundColor: AppTheme.success,
                                    duration: Duration(seconds: 1),
                                  ),
                                );
          ...
          return DefaultTabController(
            ...
            child: AlertDialog(
              ...
              content: Form(
                ...
                child: SizedBox(
                  width:
                      480, // Expand width slightly to prevent input label clippings
                  child: SingleChildScrollView(
                    child: Column(
                      mainAxisSize: MainAxisSize.min,
                      crossAxisAlignment: CrossAxisAlignment.stretch,
                      children: [
                        ...
                        Column(
                          crossAxisAlignment: CrossAxisAlignment.start,
                          children: [
                            const Text('Avg Body Weight (grams) *',
                                style: TextStyle(
                                    fontSize: 12,
                                    fontWeight: FontWeight.bold,
                                    color: AppTheme.grey600)),
                              ...
                              Expanded(
                                child: Column(
                                  children: [
                                    ...
                                    const SizedBox(height: 2),
                                    Text(
                                        '${stdConsumptionKg.toStringAsFixed(1)} KG',
                                        style: const TextStyle(
                                            fontSize: 13,
                                            fontWeight: FontWeight.bold,
                                            color: Colors.purple)),
                        ...
                        SizedBox(
                          height:
                              240, // Height expanded to avoid textboxes overflow
                          child: TabBarView(
                            physics: const NeverScrollableScrollPhysics(),
                            children: [
                                  ...
                                  children: [
                                    ...
                                    Expanded(
                                      child: capturedMedia.isEmpty
                                          ? const Center(
                                              child: Text(
                                                  'No photos/videos captured today',
                                                  style: TextStyle(
                                                      color: AppTheme.grey400,
                                                      fontSize: 11)),
                                            )
                                          : GridView.builder(
                                              ...
                                              itemBuilder: (ctx, idx) {
                                                ...
                                                final isVideo =
                                                    media.endsWith('.mp4');
                                                final double fileLat =
                                                    18.5204 + (idx * 0.002);
                                                final double fileLng =
                                                    73.8567 + (idx * 0.003);
                                                final fileTimeStr =
                                                    "${txDate.day}/${txDate.month}/${txDate.year} ${DateTime.now().hour.toString().padLeft(2, '0')}:${DateTime.now().minute.toString().padLeft(2, '0')}";
                                                                  ...
                                                                  children: [
                                                                    ...
                                                                    Text(
                                                                        ...
                                                                        overflow:
                                                                            TextOverflow.ellipsis),
                                                                    Text(
                                                                        'GPS: ${fileLat.toStringAsFixed(4)}, ${fileLng.toStringAsFixed(4)}',
                                                                        style: const TextStyle(
                                                                            color: Colors
                                                                                .greenAccent,
                                                                            fontSize:
                ...
                if (readOnly)
                  ...
                  TextButton(
                    onPressed: () {
                      if (formKey.currentState!.validate()) {
                        final bodyWeight =
                            double.tryParse(bodyWeightCtrl.text) ?? 0.0;
                        final mortVals = mortCtrls.map(
                            (k, v) => MapEntry(k, int.tryParse(v.text) ?? 0));
                        final feedVals = feedCtrls.map((k, v) =>
                        ...
                        widget.onSaveTransaction(transaction,
                            isEdit: existingTx != null);
                        _refreshTransactions();

                        Navigator.pop(ctx);
                        ScaffoldMessenger.of(context).showSnackBar(
                          SnackBar(
                            content: Text(existingTx != null
                                ? 'Daily log updated!'
...
class _DailyTransactionReportScreenState
  ...
  Widget build(BuildContext context) {
    ...
    // Totals
    final totalMortality = filteredTxs.fold(
        0,
        (sum, t) =>
            sum + t.mortalityReasons.values.fold(0, (s, val) => s + val));
    final totalFeedKg = filteredTxs.fold(0.0,
        (sum, t) => sum + t.feedItems.values.fold(0.0, (s, val) => s + val));
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        backgroundColor: AppTheme.primaryRed,
        iconTheme: const IconThemeData(color: Colors.white),
        title: const Text('Daily Transaction Report',
            style: TextStyle(
                color: Colors.white,
                fontWeight: FontWeight.bold,
                fontSize: 18)),
        actions: [
          IconButton(
            ...
            onPressed: () {
              ScaffoldMessenger.of(context).showSnackBar(
                const SnackBar(
                    content: Text('Report exported to Downloads folder!'),
                    backgroundColor: AppTheme.success),
              );
            },
          ),
        ...
        children: [
          ...
          Padding(
            ...
            child: Container(
              ...
              child: Row(
                mainAxisAlignment: MainAxisAlignment.spaceAround,
                children: [
                  _summaryItem('Mortality Count', '$totalMortality Birds',
                      AppTheme.primaryRed),
                  Container(width: 1, height: 28, color: AppTheme.grey300),
                  _summaryItem('Feed Intake',
                      '${totalFeedKg.toStringAsFixed(1)} KG', Colors.orange),
                ],
              ),
            ),
          ),
          ...
          Expanded(
            child: filteredTxs.isEmpty
                ? const Center(
                    child: Text('No daily logs found matching selection',
                ...
                : ListView.builder(
                    ...
                    itemBuilder: (ctx, idx) {
                      ...
                      final b = widget.batches.firstWhere(
                          (batch) => batch.id == tx.batchId,
                          orElse: () => PoultryBatch(
                              id: '',
                              batchName: 'Unknown',
                              module: '',
                              ...
                              breedName: '',
                              date: DateTime.now(),
                              firstDayDate: DateTime.now(),
                              stdLiftingAge: 42,
                              chicksQuantity: 0,
                              birdRate: 0.0));
                      final mort = tx.mortalityReasons.values
                          .fold(0, (sum, val) => sum + val);
                      final feed = tx.feedItems.values
                      ...
                      return Container(
                        ...
                        child: Row(
                          children: [
                            Expanded(
                                flex: 2,
                                child: Text('${tx.date.day}/${tx.date.month}',
                                    style: const TextStyle(
                                        fontSize: 12,
                                        color: AppTheme.grey800))),
                            Expanded(
                            ...
                            Expanded(
                                flex: 2,
                                child: Text(
                                    '${tx.bodyWeight.toStringAsFixed(0)}g',
                                    style: const TextStyle(
                                        fontSize: 12, color: AppTheme.grey600),
                                    textAlign: TextAlign.center)),
                            Expanded(
                            ...
                            Expanded(
                                flex: 2,
                                child: Text('${feed.toStringAsFixed(1)} kg',
                                    style: const TextStyle(
                                        fontSize: 12,
                                        color: Colors.orange,
                                        fontWeight: FontWeight.w600),
...
class _AnalyticsScreenState extends State<AnalyticsScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    final chartTxs = filteredTxs.length > 7
        ? filteredTxs.sublist(filteredTxs.length - 7)
        : filteredTxs;
    ...
    final int totalMortality =
        diseaseCount + heatCount + weaknessCount + accidentCount + otherCount;
            ...
            Card(
              ...
              child: Padding(
                ...
                child: Column(
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: [
                    const Text('Daily Feed Consumed (KG) — Trend',
                        style: TextStyle(
                            fontWeight: FontWeight.bold,
                            fontSize: 14,
                            color: AppTheme.grey800)),
                    const SizedBox(height: 4),
                    const Text('Shows feed logs of last 7 entries',
                        style:
                    ...
                    if (chartTxs.isEmpty)
                      const SizedBox(
                          height: 120,
                          child: Center(
                              child: Text('No logging entries yet',
                                  style: TextStyle(
                          ...
                          children: chartTxs.map((tx) {
                            ...
                            return Column(
                              ...
                              children: [
                                ...
                                const SizedBox(height: 6),
                                Text('${tx.date.day}/${tx.date.month}',
                                    style: const TextStyle(
            ...
            Card(
              ...
              child: Padding(
                ...
                child: Column(
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: [
                    const Text('Actual FCR vs. Cobb Standard FCR',
                        style: TextStyle(
                            fontWeight: FontWeight.bold,
                            fontSize: 14,
                            color: AppTheme.grey800)),
                    const SizedBox(height: 4),
                    const Text(
                        'Green represents actual FCR, Blue standard Cobb curves',
                        style:
                            TextStyle(fontSize: 11, color: AppTheme.grey500)),
                    const SizedBox(height: 20),
                    if (chartTxs.isEmpty)
                      ...
                      SizedBox(
                        ...
                        child: Row(
                          ...
                          children: chartTxs.map((tx) {
                            ...
                            double actualFill =
                                (actualFcr / maxFcr).clamp(0.05, 1.0);
            ...
            Card(
              ...
              child: Padding(
                ...
                child: Column(
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: [
                    const Text('Mortality Analysis — Reason Breakdown',
                        style: TextStyle(
                            fontWeight: FontWeight.bold,
                            fontSize: 14,
                            color: AppTheme.grey800)),
                    const SizedBox(height: 4),
                    Text('Total dead birds linked: $totalMortality',
                        style: const TextStyle(
                    ...
                    if (totalMortality == 0)
                      const SizedBox(
                          height: 100,
                          child: Center(
                              child: Text('0 mortality reported',
                                  style: TextStyle(
                                      fontSize: 12,
                                      color: Colors.green,
                                      fontWeight: FontWeight.bold))))
                      ...
                      _mortReasonRow(
                          'Disease', diseaseCount, totalMortality, Colors.red),
                      _mortReasonRow('Heat Stroke', heatCount, totalMortality,
                          Colors.amber),
                      _mortReasonRow('Weakness', weaknessCount, totalMortality,
                          Colors.orange),
                      _mortReasonRow('Accident', accidentCount, totalMortality,
                          Colors.brown),
                      _mortReasonRow(
                          'Other', otherCount, totalMortality, Colors.grey),
                    ],
                  ],
                ),
              ),
            ...
            children: [
              Text(reason,
                  style: const TextStyle(
                      ...
                      fontWeight: FontWeight.w600,
                      color: AppTheme.grey700)),
              Text('$count Birds (${(percent * 100).toStringAsFixed(1)}%)',
                  style: const TextStyle(
                      fontSize: 11,
                      fontWeight: FontWeight.bold,
                      color: AppTheme.grey800)),
